---
layout: about
title: about
permalink: /
subtitle: PhD Student, <a href='https://www.huck.psu.edu/graduate-programs/bioinformatics-and-genomics'>Bioinformatics and Genomics</a>, <a href='https://en.wikipedia.org/wiki/Pennsylvania_State_University'>Penn State</a>, United States
profile:
  align: right
  image: profile.jpeg
  image_circular: false
  more_info:
news: false # set to true when you add news items in _news/
selected_papers: false # set to true after you add your papers to _bibliography/papers.bib and mark them as selected
social: true # includes social icons at the bottom of the page
latest_posts: false # set to true when you start blogging
---

<style>
body, h1, h2, h3, h4, h5, h6, nav a, .navbar-brand { font-family: 'Lora', serif !important; }
.social a i { font-size: 1.5rem !important; }
:root { --global-theme-color: #1b5e20 !important; --global-hover-color: #0d3b13 !important; }
html[data-theme="dark"] { --global-theme-color: #81c784 !important; --global-hover-color: #a5d6a7 !important; }
</style>

I am a fourth year PhD student in the Bioinformatics and Genomics program at Penn State, advised by [Zachary Szpiech](http://szpiech.com/). My broad research interests fall into two areas: theoretical population genetics and evolutionary human genetics. During my PhD, I have been focusing on the theory and methods of runs of homozygosity (ROH) and am deeply curious about the landscape of deleterious mutation load. I work with both empirical and simulated data and have extensive experience with [SLiM](https://messerlab.org/slim/) for forward-in-time simulation. Beyond these core areas, I am also drawn to spatial population genetics and conservation genetics.

Before coming to Penn State, I completed my Bachelor of Science in Mathematics and Applied Mathematics at [Beijing Normal University](https://simple.wikipedia.org/wiki/Beijing_Normal_University). During the final years of my undergraduate studies, I worked with [Jackson Champer](https://jchamper.github.io/) at [Peking University](https://en.wikipedia.org/wiki/Peking_University), where I began my research career through computational simulation of gene drives.

Outside of research, I am an avid classical music listener. I'm a devoted audience of [Augustin Hadelich](https://augustinhadelich.com/) and [Krystian Zimerman](https://en.wikipedia.org/wiki/Krystian_Zimerman). I write literary fiction in Chinese, and am a faithful reader of [Kazuo Ishiguro](https://en.wikipedia.org/wiki/Kazuo_Ishiguro). I also raised two tuxedo cats from six weeks old, and they are now family.
